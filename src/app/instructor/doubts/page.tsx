...
import { MoreHorizontal, Search, MessageSquareReply, CheckCircle, Clock } from "lucide-react";
...
// Mock data, in a real app this would come from Firebase for the logged-in instructor
const mockDoubts: StudentDoubt[] = [
  { id: "d1", studentName: "Alice Wonderland", studentAvatar: "https://placehold.co/50x50.png?a=A", courseTitle: "Advanced English Grammar", doubtSummary: "Confused about subjunctive mood...", dateSubmitted: "2024-06-10", status: "Pending" },
  { id: "d2", studentName: "Carlos Ray", studentAvatar: "https://placehold.co/50x50.png?c=C", courseTitle: "Business Communication", doubtSummary: "Need clarification on email etiquette for formal complaints.", dateSubmitted: "2024-06-09", status: "Answered" },
  { id: "d3", studentName: "Fiona Gallagher", studentAvatar: "https://placehold.co/50x50.png?f=F", courseTitle: "Advanced English Grammar", doubtSummary: "Difference between 'lay' and 'lie'?", dateSubmitted: "2024-06-11", status: "In Progress" },
];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground font-body">Review and respond to questions submitted by students in your courses.</p>
      </div>
            ...
            <TableBody>
              {mockDoubts.map((doubt) => (
                <TableRow key={doubt.id}>
                  <TableCell className="font-medium font-body">
                     <div className="flex items-center space-x-2">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={doubt.studentAvatar} alt={doubt.studentName} data-ai-hint="student avatar"/>
                  ...
                  <TableCell>
                    <Badge 
                        variant={doubt.status === "Answered" ? "default" : doubt.status === "Pending" ? "destructive" : "secondary"}
                        className={`capitalize font-body ${
                            doubt.status === "Answered" ? 'bg-green-500/20 text-green-700 border-green-400' 
                            : doubt.status === "Pending" ? 'bg-red-500/20 text-red-700 border-red-400'
                            : 'bg-yellow-500/20 text-yellow-700 border-yellow-400'}`} // In Progress
                    >
                        {doubt.status === "Answered" && <CheckCircle className="inline-block h-3 w-3 mr-1" />}
                        {doubt.status === "Pending" && <MessageSquareWarning className="inline-block h-3 w-3 mr-1" />}
                        {doubt.status === "In Progress" && <Clock className="inline-block h-3 w-3 mr-1" />}
                        {doubt.status}
                    </Badge>
                  </TableCell>
